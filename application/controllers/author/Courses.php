...
class Courses extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['info'] = array(
			'page_title' => 'Instructor - Cursos',
			'title' => 'Cursos',
			'sub_title' => 'Administrar cursos',
			'active' => 2,
		...
		$data['categories'] = $this->course->listCategoryforUser($this->session->userdata('id'), 'category', 'user_id');
...
//		$this->form_validation->set_rules('category_id', 'category_id', 'trim');
		...
		$this->form_validation->set_rules('price', 'precio', 'trim|decimal');
		$this->form_validation->set_rules('discount_price', 'descuento', 'trim|decimal');
		...
		if ($this->form_validation->run() == FALSE) :
			...
			$data['info'] = array(
				'page_title' => 'Instructor - Nuevo curso',
				'title' => 'Cursos',
			...
			redirect(base_url() . 'author/courses/edit/' . $crs, 'location');
	...
	public function edit($id = 0, $author = 0)
	{
		...
		if (isset($id) and !empty($id)):
			...
			$this->form_validation->set_rules('video_url', 'video_url', 'trim');
			$this->form_validation->set_rules('is_free_course', 'is_free_course', 'trim');
			$this->form_validation->set_rules('price', 'price', 'trim|decimal');
			...
			if ($this->form_validation->run() == FALSE) :
				...
				redirect(base_url() . 'author/courses/edit/' . $id . '/' . $author, 'location');
			endif;
		else:
			$this->session->set_flashdata('message', '¡Ubicación del curso invalido!');
			redirect(base_url() . 'author/courses', 'location');
		endif;
	}
